...
const (
	// googleWorkspaceSyncInterval is how often Fleet pulls the Google Workspace
	// directory. Directory data changes slowly, so an hourly pull is plenty.
	googleWorkspaceSyncInterval = 1 * time.Hour
...
func cronGoogleWorkspaceSync(ctx context.Context, ds fleet.Datastore, factory GoogleWorkspaceDirectoryFactory, logger *slog.Logger) error {
	...
	if syncErr != nil {
		...
		lastRequest.Details = syncErr.Error()
